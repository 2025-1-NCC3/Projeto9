...
public class TesteBackEnd extends AppCompatActivity {
            ...
            public void onResponse(Call<List<Usuario>> call, Response<List<Usuario>> response) {
                ...
                if (response.isSuccessful()) {
                    ...
                    if (usuarios != null && !usuarios.isEmpty()) {
                        // Descriptografa o primeiro usu''''''''''''ário apenas para demonstração
                        Usuario user = decryptFromServer(usuarios.get(7));
                        resultadoConexao.setText("Conexão bem-sucedida! Primeiro usuário: " + user.getNome());
                    } else {
                        resultadoConexao.setText("Conexão bem-sucedida! Nenhum usuário encontrado.");
                    }
    ...
    private void enviarDadosParaServidor() {
        ...
        nome = encryptForServer(nome);
        email = encryptForServer(email);
